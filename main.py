from teкгтlebot import types
import telebot

bot = telebot.TeleBot('5832076627:AAG-OQHssKQ68lo_lLk3mufEla0ij0iPq1U')


def webAppKeyboard():  # создание клавиатуры с webapp кнопкой
    keyboard = types.ReplyKeyboardMarkup(row_width=1)  # создаем клавиатуру
    # создаем webappinfo - формат хранения url
    webAppTest = types.WebAppInfo("https://neferuz.uz/")
    # создаем webappinfo - формат хранения url
    webAppGame = types.WebAppInfo("https://games.mihailgok.ru")
    one_butt = types.KeyboardButton(
        text="Тестовая страница", web_app=webAppTest)  # создаем кнопку типа webapp
    two_butt = types.KeyboardButton(
        text="Игра", web_app=webAppGame)  # создаем кнопку типа webapp
...
def webAppKeyboardInline():  # создание inline-клавиатуры с webapp кнопкой
    keyboard = types.InlineKeyboardMarkup(
        row_width=1)  # создаем клавиатуру inline
    # создаем webappinfo - формат хранения url
    webApp = types.WebAppInfo("https://bellissimo.uz/")
    one = types.InlineKeyboardButton(
        text="Веб приложение", web_app=webApp)  # создаем кнопку типа webapp
